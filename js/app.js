(() => {
    ...
    da.init();
    const columnsContainer = document.querySelector(".columns__container");
    if (columnsContainer) {
        ...
        if (columns) {
            ...
            columns.forEach((element => {
                index++;
                element.classList.add(`column_${index}`);
                if (index <= 9) {
                    const elementNumberSpan = element.querySelector(".column__number span");
                    console.log(elementNumberSpan);
                    elementNumberSpan.innerHTML = `0${index}`;
                }
                if (index > 9) {
                    const elementNumberSpan = element.querySelector(".column__number span");
                    console.log(elementNumberSpan);
                    elementNumberSpan.innerHTML = `${index}`;
                }
            }));
        }
    }
    const currentVideo = document.querySelector(".video__current-video");
    if (currentVideo) {
        ...
        currentVideo.addEventListener("timeupdate", (function(e) {
            ...
            const progressWidth = currentVideoTime / videoDuration * 100;
            progressBar.style.width = `${progressWidth}%`;
        }));
        const progressArea = document.querySelector(".video__progress-area");
        progressArea.addEventListener("click", (function(e) {
            const videoDuration = currentVideo.duration;
            const progressWidth = progressArea.clientWidth;
            const clickCoordX = e.offsetX;
            console.log(clickCoordX);
            currentVideo.currentTime = clickCoordX / progressWidth * videoDuration;
        }));
    }
    document.addEventListener("click", documentActions);
    function documentActions(e) {
        const targetElement = e.target;
        if (targetElement.closest(".video__inner")) {
            const videoButton = document.querySelector(".video__play-pause");
            const currentVideo = document.querySelector(".video__current-video");
            ...
            if (videoButton.classList.contains("play")) {
                currentVideo.play();
                videoTime.classList.add("_active");
            }
            if (!videoButton.classList.contains("play")) {
                currentVideo.pause();
                videoTime.classList.remove("_active");
            }
        }
    }
    window["FLS"] = true;
